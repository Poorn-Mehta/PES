...
int t; // test integer
int *ptr, *ptr_2, *ptr_3, *ptr_4, *ptr_5, *ptr_6, *ptr_7; //Pointer for memory operations
...
void generator(void)
{
	...
	printf("\n%d ",print);
							*(ptr+i-1) = print;
						printf("\nRelative address: %d\t\t Actual Address: %x\t\t\t Data stored at this address: %d", i, (ptr+i-1), *(ptr+i-1));
	seed = random;
}
...
// 9. Time measurement for any operation(clk_begin): clkbegin(void)
