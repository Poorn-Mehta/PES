...
void memwrite(void);
void memxor(void);
void psuedogen(void);	void generator(void); void generator_store(void); //Internal function
//void psuedover(void);
//void memtest(void);
//void timetest(void);
...
P1:				printf("\nEnter the number of 32-bits that are to be allocated which should be greater than or equal to the number of psuedo-random numbers that are generated:\n");
				...
				if(n>=n_2)
				{
					...
					for(i=1; i<=n_2; i++)
					{
						...
						*(ptr_2+i-1) = print;
						printf("\nRelative address: %d\t\t Actual Address: %x\t\t Data stored at this address: %x", i, (ptr_2+i-1), *(ptr_2+i-1));
						seed = random;
					}
				}
				else
...
P2:					printf("\nEnter the number of 32-bits that are to be allocated which should be greater than or equal to the number of psuedo-random numbers that are generated:\n");
		...
		{
			printf("\nThe generated psuedo-random number is not stored as per your wish\n");
		}
//			else if(n < n_2)
//			{
